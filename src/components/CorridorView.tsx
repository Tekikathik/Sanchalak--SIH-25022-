    ...
    <svg viewBox={`0 0 ${VB_W} ${VB_H}`} className="h-full w-full" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Live corridor control board">
      ...
      {snap.trains
        ...
        .map((t) => {
          ...
          return <TrainGlyph key={t.id} x={sx(km)} y={y} t={t} selected={selectedId === t.id} onSelect={onSelect} playing={playing} speed={speed} />
        })}
    </svg>
  )
}
...
function TrainGlyph({
  ...
  const transition = stationParked || !playing ? 'none' : `transform ${Math.max(0.06, 0.16 / speed)}s linear`
  return (
    <g
      onClick={() => onSelect(selected ? null : t.id)}
      style={{ transform: `translate(${x - w / 2}px, ${y - h / 2}px)`, transition, cursor: 'pointer' }}
    >
