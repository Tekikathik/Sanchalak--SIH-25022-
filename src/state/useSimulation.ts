    ...
    (ts: number) => {
      ...
      if (playingRef.current) {
        ...
        while (budget > 1e-6) {
          ...
          if (!sims.current.opt.isDone()) sims.current.opt.step(step)
          if (!sims.current.fcfs.isDone()) sims.current.fcfs.step(step)
          budget -= step
        }
        // Publish at ~12fps; motion stays smooth while React rendering overhead is minimized
        const done = sims.current.opt.isDone() && sims.current.fcfs.isDone()
