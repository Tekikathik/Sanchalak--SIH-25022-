...
export class Simulation {
  ...
  private loopFreeFor = (stationId: string, trainId: string): boolean => {
    const st = this.index.stationById.get(stationId)
    if (!st) return true
    const alreadyOnLoop = this.loopHeld.get(stationId)?.has(trainId) ?? false
    if (alreadyOnLoop) return true
    return this.loopCountAt(stationId) < st.loopLines
  }
  ...
  isDone(): boolean {
    for (const t of this.trains) {
      if (t.state !== 'ARRIVED') return false
    }
